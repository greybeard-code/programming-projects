		...
		public GreyBeard.gbNobleCloud gbNobleCloud(ISeries<double> input, double sensitivity, int smoothness, gbNobleCloud_MAType baselineMAType, int baselinePeriod, bool baselineSmoothingEnabled, gbNobleCloud_MAType baselineSmoothingMethod, int baselineSmoothingPeriod, gbNobleCloud_MAType kernelMAType, int kernelPeriod, bool kernelSmoothingEnabled, gbNobleCloud_MAType kernelSmoothingMethod, int kernelSmoothingPeriod, int signalSplit, bool filterEnabled, int filterBarMin, int filterBarMax)
		{
			if (cachegbNobleCloud != null)
				for (int idx = 0; idx < cachegbNobleCloud.Length; idx++)
					if (cachegbNobleCloud[idx] != null && cachegbNobleCloud[idx].Sensitivity == sensitivity && cachegbNobleCloud[idx].Smoothness == smoothness && cachegbNobleCloud[idx].BaselineMAType == baselineMAType && cachegbNobleCloud[idx].BaselinePeriod == baselinePeriod && cachegbNobleCloud[idx].BaselineSmoothingEnabled == baselineSmoothingEnabled && cachegbNobleCloud[idx].BaselineSmoothingMethod == baselineSmoothingMethod && cachegbNobleCloud[idx].BaselineSmoothingPeriod == baselineSmoothingPeriod && cachegbNobleCloud[idx].KernelMAType == kernelMAType && cachegbNobleCloud[idx].KernelPeriod == kernelPeriod && cachegbNobleCloud[idx].KernelSmoothingEnabled == kernelSmoothingEnabled && cachegbNobleCloud[idx].KernelSmoothingMethod == kernelSmoothingMethod && cachegbNobleCloud[idx].KernelSmoothingPeriod == kernelSmoothingPeriod && cachegbNobleCloud[idx].SignalSplit == signalSplit && cachegbNobleCloud[idx].FilterEnabled == filterEnabled && cachegbNobleCloud[idx].FilterBarMin == filterBarMin && cachegbNobleCloud[idx].FilterBarMax == filterBarMax && cachegbNobleCloud[idx].EqualsInput(input))
						return cachegbNobleCloud[idx];
			return CacheIndicator<GreyBeard.gbNobleCloud>(new GreyBeard.gbNobleCloud(){ Sensitivity = sensitivity, Smoothness = smoothness, BaselineMAType = baselineMAType, BaselinePeriod = baselinePeriod, BaselineSmoothingEnabled = baselineSmoothingEnabled, BaselineSmoothingMethod = baselineSmoothingMethod, BaselineSmoothingPeriod = baselineSmoothingPeriod, KernelMAType = kernelMAType, KernelPeriod = kernelPeriod, KernelSmoothingEnabled = kernelSmoothingEnabled, KernelSmoothingMethod = kernelSmoothingMethod, KernelSmoothingPeriod = kernelSmoothingPeriod, SignalSplit = signalSplit, FilterEnabled = filterEnabled, FilterBarMin = filterBarMin, FilterBarMax = filterBarMax }, input, ref cachegbNobleCloud);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.GreyBeard.gbNobleCloud gbNobleCloud(double sensitivity, int smoothness, gbNobleCloud_MAType baselineMAType, int baselinePeriod, bool baselineSmoothingEnabled, gbNobleCloud_MAType baselineSmoothingMethod, int baselineSmoothingPeriod, gbNobleCloud_MAType kernelMAType, int kernelPeriod, bool kernelSmoothingEnabled, gbNobleCloud_MAType kernelSmoothingMethod, int kernelSmoothingPeriod, int signalSplit, bool filterEnabled, int filterBarMin, int filterBarMax)
		{
		...
		public Indicators.GreyBeard.gbNobleCloud gbNobleCloud(ISeries<double> input , double sensitivity, int smoothness, gbNobleCloud_MAType baselineMAType, int baselinePeriod, bool baselineSmoothingEnabled, gbNobleCloud_MAType baselineSmoothingMethod, int baselineSmoothingPeriod, gbNobleCloud_MAType kernelMAType, int kernelPeriod, bool kernelSmoothingEnabled, gbNobleCloud_MAType kernelSmoothingMethod, int kernelSmoothingPeriod, int signalSplit, bool filterEnabled, int filterBarMin, int filterBarMax)
		{
			return indicator.gbNobleCloud(input, sensitivity, smoothness, baselineMAType, baselinePeriod, baselineSmoothingEnabled, baselineSmoothingMethod, baselineSmoothingPeriod, kernelMAType, kernelPeriod, kernelSmoothingEnabled, kernelSmoothingMethod, kernelSmoothingPeriod, signalSplit, filterEnabled, filterBarMin, filterBarMax);
		}
	}
